...
module AsyncSeq = 
  ...
  let iterAsync (f: 'T -> Async<unit>) (inp: AsyncSeq<'T>) = 
    iteriAsync (fun i x -> f x) inp
  
  let iteri (f: int -> 'T -> unit) (inp: AsyncSeq<'T>) = 
    iteriAsync (fun i x -> async.Return (f i x)) inp
  ...
  let replicateAsync (count:int) (v:Async<'T>) : AsyncSeq<'T> =    
    asyncSeq { 
        for i in 1 .. count do 
           let! v = v
           yield v }
